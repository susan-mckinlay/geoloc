...
def prepare_dataset(data, season, indiv):
    """
    Function to drop duplicates of 'modelat' and 'modelon' and create a list of tuples with coordinates
    """
...
def select_shortest_distance(points_indiv_1, points_indiv_2):
    ...
    for point_track_a in points_indiv_1:
        for point_track_b in points_indiv_2:
            #print('this is point Track A', point_track_a, 'this is point Track B', point_track_b)
            distance = calculate_distance(point_track_a, point_track_b)
            #print('calculated distance', distance, i)
        ...
        # appends the distances of one point with all the other 20 points, it empties when it gets to point 2
        # of Track A
        tot_distance_list = []
        #print('min distance list', min_distance_list, j)
        j += 1
    print(min_distance_list)
    return min_distance_list

def interpolate_lat_and_long(data_indiv, interpolated_lat_45, interpolated_long_45, interpolated_lat_17, interpolated_long_17):
    """
...
def find_closest_values(first_row, limit, data_indiv, interpolated_lat, interpolated_long):
    ...
    difference = np.abs(limit - first_row['modelat'])
...
def place_equally_distanced_points(path, n_samples):
    ...
    geod = Geodesic.WGS84
    path_distance = [0] # the distance in mbetween each coordinate (the original ones)
    ...
    # Cumulative sum of each distance segment
    ...
    #print('pah distance cum', path_distance_cum, 'point distance', point_distance, 'path distance', path_distance)
...
def main():
    ...
    interpolated_long_17 = 13.74
    interpolated_lat_45 = 45
    interpolated_long_45 = 16.63
    data_indiv_5IK = interpolate_lat_and_long(data_indiv_5IK, interpolated_lat_45, interpolated_long_45, interpolated_lat_17, interpolated_long_17)
    ...
    interpolated_lat_17 = 17
    interpolated_long_17 = 10.141
    interpolated_lat_45 = 45
    interpolated_long_45 = 9.69
    data_indiv_5LK = interpolate_lat_and_long(data_indiv_5LK, interpolated_lat_45, interpolated_long_45, interpolated_lat_17, interpolated_long_17)
    ...
    tot_dist_migr = calc_dist_each_rand_loc(data, season)
    sum_5IK = sum_distances_5IK/tot_dist_migr.loc[tot_dist_migr['ID'] == '5LK', 'distance'].values[0]
    ...
    sum_5LK = sum_distances_5LK/tot_dist_migr.loc[tot_dist_migr['ID'] == '5IK', 'distance'].values[0]
    print(len(min_distance_list_5IK), len(min_distance_list_5LK))
    # Final one-way distance
    print('final one-way distance in km', sum_distances_5IK, sum_distances_5LK, sum_5IK, sum_5LK, (sum_5IK + sum_5LK)/2)
